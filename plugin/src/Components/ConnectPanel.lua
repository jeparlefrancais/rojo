...
local Miniotter = require(Plugin.Miniotter)
local spring = require(Plugin.spring)

local DialogBackdrop = require(Plugin.Components.DialogBackdrop)
...
function ConnectPanel:didMount()
	local goal = spring(0.5, { frequency = 3 })
	self.otter:setGoal(goal, function(didFinish)
		print("tada", didFinish)
	end)
end
	...
	return e(DialogBackdrop, {
		...
		Content = e(FitList, {
			...
			containerProps = {
				...
				Position = self.otter.binding:map(function(yPosition)
